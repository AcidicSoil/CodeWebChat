...
export const chatgpt_custom: Chatbot = {
  ...
  set_options: async (options?: string[]) => {
    if (!options) return
    const supported_options = CHATBOTS['ChatGPT Custom'].supported_options
  ...
  inject_apply_response_button: (client_id: number) => {
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach(() => {
        ...
        show_response_ready_notification({ chatbot_name: 'ChatGPT Custom' })
